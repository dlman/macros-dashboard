...
function renderStatCards() {
  ...
  [...metrics, ...perLb].forEach(m => {
    const vals = ACTIVE_MONTHS.map(mo => {
      const d = filtered[mo.key];
    ...
    let arrows = Array(ACTIVE_MONTHS.length).fill('');
    ...
    card.innerHTML = `
      ...
      <div class="months">
        ${ACTIVE_MONTHS.map((mo, i) => `<div class="month-val"><span class="month-name">${mo.label}</span><span class="val" style="color:${mo.color}">${vals[i] == null ? '—' : m.fmt(vals[i])}${arrows[i]}</span></div>`).join('')}
      </div>`;
  ...
  const mismatchVals = ACTIVE_MONTHS.map(mo => {
    const mDays = filtered[mo.key];
    if (!mDays || !mDays.length) return null;
  ...
  mismatchCard.innerHTML = `
    ...
    <div class="months">
      ${ACTIVE_MONTHS.map((mo, i) => `<div class="month-val"><span class="month-name">${mo.label}</span><span class="val" style="font-size:16px;color:${mismatchVals[i] && Math.abs(mismatchVals[i].gap) > 5 ? 'var(--col-amber)' : 'var(--text-muted)'}">${mismatchVals[i] == null ? '—' : (mismatchVals[i].gap > 0 ? '+' : '') + mismatchVals[i].gap.toFixed(1) + '%'}</span></div>`).join('')}
    </div>`;
  ...
  consistencyCard.innerHTML = `
    ...
    <div class="months">
      ${ACTIVE_MONTHS.map(mo => {
        const d = filtered[mo.key];
        const calCon = d && d.length >= 3 ? consistencyScore(d, 'calories') : null;
        const proCon = d && d.length >= 3 ? consistencyScore(d, 'protein') : null;
        ...
        return `<div class="month-val"><span class="month-name">${mo.label}</span><span class="val" style="font-size:13px;"><span style="color:${calColor}">${calCon != null ? calCon + '%' : '—'}</span> / <span style="color:${proColor}">${proCon != null ? proCon + '%' : '—'}</span></span></div>`;
      }).join('')}
    </div>
...
function renderSleepStatCards() {
  ...
  metrics.forEach(m => {
    const vals = ACTIVE_MONTHS.map(mo => {
      const d = sleepMonths[mo.key] || [];
      return m.computed ? m.computed(d) : avgOrNull(d, m.key);
    });
    let arrows = Array(ACTIVE_MONTHS.length).fill('');
    for (let i = 1; i < ACTIVE_MONTHS.length; i++) {
      if (vals[i] == null || vals[i - 1] == null) continue;
    ...
    const card = document.createElement('div');
    card.className = 'stat-card';
    card.innerHTML = `<div class="label">${m.label}</div><div class="months">${ACTIVE_MONTHS.map((mo,i) => `<div class="month-val"><span class="month-name">${mo.label}</span><span class="val" style="color:${mo.color}">${vals[i] == null ? '—' : m.fmt(vals[i])}${arrows[i]}</span></div>`).join('')}</div>`;
    grid.appendChild(card);
  });
}
...
function renderForecastStrip(filteredDays, filteredSleep) {
  ...
  if (milestoneEl) {
    if (!bfTargets.length) {
      ...
      const scanAnchored = bfTargets[0].anchorDate >= DXA_SCAN_LATEST.date;
      ...
      const milestoneHtml = bfTargets.map(target => {
        ...
        const primaryWeight = scanAnchored
          ? targetWeightRangeLabel(target, 'fedLow', 'fedHigh', target.fedStateTargetWeight)
          : targetWeightRangeLabel(target, 'cutLow', 'cutHigh', target.cutStateTargetWeight);
        const comparableWeight = scanAnchored
          ? targetWeightRangeLabel(target, 'cutLow', 'cutHigh', target.cutStateTargetWeight)
          : targetWeightRangeLabel(target, 'fedLow', 'fedHigh', target.fedStateTargetWeight);
        return `
          <div class="bf-milestone ${state}" style="--milestone-left:${targetPct}%">
            ...
            <div class="bf-ms-weight">${primaryWeight} ${scanAnchored ? 'fed' : 'cut'}</div>
            <div class="bf-ms-fed">${comparableWeight} ${scanAnchored ? 'cut equivalent' : 'fed'}</div>
          </div>
        `;
      }).join('');
      milestoneEl.innerHTML = `
        <div class="bf-strip-copy">
          ...
          <div class="bf-strip-sub">${nextTarget ? `Next: ${nextTarget.targetBfPct}% around ${targetWeightRangeLabel(nextTarget, scanAnchored ? 'fedLow' : 'cutLow', scanAnchored ? 'fedHigh' : 'cutHigh', scanAnchored ? nextTarget.fedStateTargetWeight : nextTarget.cutStateTargetWeight)} ${bodyCompStateLabel(bfTargets[0].anchorDate)}.` : 'All displayed milestones are inside the current estimate.'} Based on ${bfTargets[0].currentWeightAnchor || 'current trend weight'}.${scanAnchored ? ` ${formatShortDate(DXA_SCAN_LATEST.date)} is calibrated as fed-state; creatine is already included and the cut-state equivalent is ${weightLabel(DXA_SCAN_LATEST.fedStateDelta, 1)} lower.` : creatineWater > 0 ? ` Scale includes ~${creatineRangeLabel} modeled creatine water; BF math uses the ${weightLabel(creatineWater, 1)} midpoint and excludes it from fat trend.` : ''}</div>
        </div>
  ...
  if (runwayEl) {
    ...
    if (!runway) {
      ...
      const statusCopy = {
        ...
        'on-track': { label: 'On track', detail: runway.bufferDays > 0 ? `Projected ${runway.bufferDays} days before the working deadline.` : 'Current pace clears the working deadline.', cls: 'on-track' },
        narrow: { label: 'Narrow margin', detail: 'Current pace is close to the minimum required for the working deadline.', cls: 'narrow' },
      ...
      runwayEl.innerHTML = `
        <div class="runway-lead">
          <div class="eyebrow">${runway.targetBfPct}% working deadline · Nov 15</div>
        ...
        <div class="runway-metrics">
          <div class="runway-metric">
            ...
            <small>to ${weightLabel(runway.targetWeight, 1)} ${bodyCompStateLabel(runway.anchorDate)}${runway.targetCutWeight != null && runway.targetCutWeight !== runway.targetWeight ? ` · ${weightLabel(runway.targetCutWeight, 1)} cut equivalent` : ''}</small>
          </div>
          <div class="runway-metric">
...
const weightPoints = [];
ACTIVE_MONTHS.forEach(m => {
  data[m.key].filter(d=>d.weight).forEach(d => weightPoints.push({ x: d.date.slice(5)+' ('+m.label+')', y: d.weight, date: d.date }));
});
...
// Glycogen-adjusted weight: removes the glycogen+water delta vs. the Jan 6 DXA reference.
// Interpretation: "what would scale say if you were carrying the same glycogen as Jan 6?"
// When the line is ABOVE actual weight → you're more depleted now than at reference.
const glycoAdjWeights = weightPoints.map(p => {
  const s = glycogenByDate[p.date];
  if (!s) return null;
  const delta = s.massLbs - glycogenRefState.massLbs; // positive = carrying more glyco/water now
  return +( (p.y - delta) ).toFixed(1);
});
...
allCharts.weightChart = new Chart(document.getElementById('weightChart'), {
  ...
  options: {
    ...
    plugins: {
      ...
      tooltip: { ...chartDefaults().plugins.tooltip, callbacks: {
        label: ctx => {
          if (ctx.datasetIndex === 0) return ` ${ctx.parsed.y} ${weightUnit()}`;
          if (ctx.datasetIndex === 1) return ` 7d avg: ${ctx.parsed.y.toFixed(1)} ${weightUnit()}`;
          const s = glycogenByDate[weightDates[ctx.dataIndex]];
          return s ? ` Glyco-adj: ${ctx.parsed.y} ${weightUnit()} (glycogen ${s.loadPct}% loaded)` : '';
        }
      }}
    },
    ...
    plugins: { ...chartDefaults().plugins,
      ...
      tooltip: { ...chartDefaults().plugins.tooltip, filter: ctx => ![0, 1].includes(ctx.datasetIndex), callbacks: {
        ...
        afterBody: ctx => {
          ...
          if (d.measured) {
            const refLoad = glycogenRefState.loadPct;
            const scanDelta = gs ? (gs.loadPct - refLoad).toFixed(1) : null;
...
// =====================================================================
(() => {
  const el = document.getElementById('glycogenChart');
  if (!el || !glycogenStates.length) return;

  // Colour each point by load level: green ≥70 %, amber 40–70 %, red <40 %
  const loadColors = glycogenStates.map(s =>
    s.loadPct >= 70 ? 'rgba(52,211,153,0.55)' :
    s.loadPct >= 40 ? 'rgba(251,191,36,0.55)' :
                      'rgba(248,113,113,0.55)'
  );

  // Mark the two DXA scan dates
  const dxaDates = DXA_SCANS.map(scan => scan.date);
  const dxaLabels = Object.fromEntries(DXA_SCANS.map(scan => [scan.date, `DXA ${formatShortDate(scan.date)}`]));

  allCharts.glycogenChart = new Chart(el, {
    type: 'line',
    data: {
      labels: glycogenStates.map(s => s.date.slice(5)),
      datasets: [
        {
          label: 'Glycogen Load %',
          data: glycogenStates.map(s => s.loadPct),
          borderColor: 'rgba(192,132,252,0.9)',
          backgroundColor: loadColors,
          pointBackgroundColor: loadColors,
          pointRadius: glycogenStates.map(s => dxaDates.includes(s.date) ? 8 : 2),
          pointStyle: glycogenStates.map(s => dxaDates.includes(s.date) ? 'triangle' : 'circle'),
          tension: 0.4, fill: false, borderWidth: 2
        },
        // Shaded fill zone — draw a zero-opacity area so we can colour the background
        {
          label: '_fill',
          data: glycogenStates.map(() => 100),
      ...
      plugins: {
        ...
        tooltip: { ...chartDefaults().plugins.tooltip, callbacks: {
          ...
          label: ctx => {
            ...
            const s = glycogenStates[ctx.dataIndex];
            if (!s) return '';
            const lines = [
            ...
            if (dxaDates.includes(s.date)) lines.unshift(` 📍 ${dxaLabels[s.date]}`);
            return lines;
          }
        }}
      },
      scales: {
        x: { ...chartDefaults().scales.x, ticks: { ...TICK(), maxTicksLimit: 18 } },
        y: {
          ...chartDefaults().scales.y, min: 0, max: 100,
          ticks: { ...TICK(), stepSize: 20, callback: v => v + '%' },
          title: { display: true, text: 'Glycogen Load %', color: '#94a3b8', font: { size: 10 } }
        }
      }
    }
  });
  ...
  const latest = glycogenStates[glycogenStates.length - 1];
  const jan6   = glycogenByDate[DXA_SCAN_PREV.date];
  const apr8   = glycogenByDate[DXA_SCAN.date];
  const noteEl = document.getElementById('glycogenNote');
  if (noteEl && latest) {
    noteEl.textContent = `Current: ${latest.loadPct}% loaded (~${latest.massLbs} lbs glyco+water)`;
  }
  const compEl = document.getElementById('glycogenDxaCompare');
  if (compEl && jan6 && apr8) {
    const lbsDiff = (apr8.massLbs - jan6.massLbs).toFixed(2);
    const sign = lbsDiff > 0 ? '+' : '';
    compEl.innerHTML = `
      <span>Jan 6 DXA: <strong>${jan6.loadPct}%</strong> loaded (${jan6.massLbs} lbs glyco+water)</span>
      <span>Apr 8 DXA: <strong>${apr8.loadPct}%</strong> loaded (${apr8.massLbs} lbs glyco+water)</span>
      <span>Difference: <strong>${sign}${lbsDiff} lbs</strong> — estimated glycogen/water artifact between scans</span>
      <span style="color:var(--col-green)">Glycogen-corrected lean mass on Apr 8: ~<strong>${(DXA_SCAN.leanMass - parseFloat(lbsDiff)).toFixed(1)} lbs</strong></span>`;
  }
})();
...
// =====================================================================
const monthOrder = ACTIVE_MONTHS.map(m => m.key);
let calVisibility = Array.isArray(persistedState.calVisibility) && persistedState.calVisibility.length === ACTIVE_MONTHS.length ? persistedState.calVisibility : Array(ACTIVE_MONTHS.length).fill(true);
...
let macroVisibility = Array.isArray(persistedState.macroVisibility) && persistedState.macroVisibility.length === ACTIVE_MONTHS.length ? persistedState.macroVisibility : Array(ACTIVE_MONTHS.length).fill(true);
const metricBounds = { protein:{min:80,max:250,step:20}, carbs:{min:40,max:310,step:30}, fat:{min:10,max:160,step:15} };

const macroDatasets = () => {
  const ds = ACTIVE_MONTHS.map(m => ({
    label: m.label,
    data: data[m.key].map(d => d[currentMetric]),
    borderColor: m.color,
    tension: 0.3, pointRadius: 4, pointHoverRadius: 7, fill: false, borderWidth: 2
  }));
...
// Dynamically generate donut chart cards for every active month
(function initDonutCharts() {
  const container = document.getElementById('donutChartsContainer');
  if (!container) return;
  container.innerHTML = '';
  ACTIVE_MONTHS.forEach(m => {
    const canvasId = 'donut' + m.key;
    const card = document.createElement('div');
    card.className = 'chart-card';
    card.innerHTML = `<div class="card-header"><h3>${m.longLabel} — macro split</h3></div><div class="chart-wrapper medium"><canvas id="${canvasId}"></canvas></div>`;
    container.appendChild(card);
    donutChart(canvasId, m.key);
  });
})();

// Dynamically generate month toggle buttons for calories + macro charts
(function initMonthToggles() {
  ['calToggles', 'macroToggles'].forEach((containerId, chartIdx) => {
    const chartName = chartIdx === 0 ? 'calories' : 'macro';
    const container = document.getElementById(containerId);
    if (!container) return;
    container.innerHTML = '';
    ACTIVE_MONTHS.forEach((m, idx) => {
      const btn = document.createElement('button');
      btn.className = 'mtoggle';
      btn.dataset.chart = chartName;
      btn.dataset.idx = idx;
...
function runScenarioPlanner() {
  ...
  const scanState = currentStates.primaryState === 'fed';
  const projectedComp = scanState ? projectedStates.fedState : projectedStates.cutState;
  const projectedComparable = scanState ? projectedStates.cutState : projectedStates.fedState;
  const currentComp = scanState ? currentStates.fedState : currentStates.cutState;
  const stateLabel = bodyCompStateLabel(currentComp.anchorDate, scanState ? 'fed' : 'cut');
  const projectedFatChange = +(currentComp.fat - projectedComp.fat).toFixed(1);
  const projectedLeanChange = +(projectedComp.lean - currentComp.lean).toFixed(1);
  ...
  if (scenarioPlannerMode === 'goal') {
    if (!goalProjection?.achievable) {
      ...
      html += scanState
        ? `<div>Target: <strong>${weightLabel(goalProjection.fedTargetWeight)}</strong> fed-state or <strong>${weightLabel(goalProjection.cutTargetWeight)}</strong> cut-state equivalent. Creatine is already included in the scan anchor.</div>`
        : `<div>Target: <strong>${weightLabel(goalProjection.cutTargetWeight)}</strong> ${stateLabel} or <strong>${weightLabel(goalProjection.fedTargetWeight)}</strong> fuller fed-state at the same BF.</div>`;
    ...
    html += scanState
      ? `<div>Cut-state equivalent: <strong>${weightLabel(projectedComparable.weight)}</strong>; the ${weightLabel(Math.abs(reboundEnd), 1)} difference is modeled non-fat scale mass, not additional fat loss.</div>`
      : `<div>Fed-state comparable: <strong>${weightLabel(projectedComparable.weight)}</strong> at <strong>~${projectedComparable.bodyFatPct.toFixed(1)}%</strong> on the fuller Jan-like DXA bracket; the dashed path currently layers about <strong>+${weightLabel(reboundEnd, 1)}</strong> of rebound toward that state.</div>`;
  ...
  const horizonCards = [
    ...
    },
    {
      ...
      sub: `Estimated fat change in ${stateLabel} terms · lean ${projectedLeanChange >= 0 ? '+' : '−'}${weightLabel(Math.abs(projectedLeanChange), 1)}`
    },
    {
      value: `${weightLabel(cutLowEnd)}–${weightLabel(cutHighEnd)}`,
      sub: `Likely ${stateLabel} endpoint band from the active TDEE range`
    },
    {
      value: weightLabel(parseFloat(r.projectedWeight)),
      sub: `${stateLabel[0].toUpperCase()}${stateLabel.slice(1)} endpoint from current ${weightLabel(r.currentWeight)}`
    },
    {
      value: `~${projectedComp.bodyFatPct.toFixed(1)}%`,
      sub: `${stateLabel[0].toUpperCase()}${stateLabel.slice(1)} BF · likely ${projectedComp.bodyFatPctLow.toFixed(1)}%–${projectedComp.bodyFatPctHigh.toFixed(1)}% from ~${weightLabel(projectedComp.fat, 1)} fat and ~${weightLabel(projectedComp.lean, 1)} lean`
    },
    {
      value: weightLabel(projectedComparable.weight),
      sub: scanState
        ? `Cut-state equivalent endpoint · ${weightLabel(Math.abs(reboundEnd), 1)} modeled non-fat scale difference`
        : `Fed-state comparable endpoint on the fuller Jan-like DXA bracket · dashed line layers ~+${weightLabel(reboundEnd, 1)} toward it`
    },
    {
  ...
  const goalCards = goalProjection?.achievable ? [
    ...
    {
      value: weightLabel(scanState ? goalProjection.fedTargetWeight : goalProjection.cutTargetWeight),
      sub: `${scanState ? 'Fed-state' : 'Creatine-adjusted cut-state'} target weight at ${targetBfPct.toFixed(1)}% BF`
    },
    {
      value: weightLabel(scanState ? goalProjection.cutTargetWeight : goalProjection.fedTargetWeight),
      sub: scanState
        ? `Cut-state equivalent at the same modeled fat mass`
        : `Creatine-adjusted fuller fed-state target weight at the same BF`
    },
    {
      value: `~${goalProjection.currentBfPct.toFixed(1)}%`,
      sub: `Current ${stateLabel} BF estimate from the DXA bracket`
    },
    {
    ...
    {
      value: weightLabel(Math.abs((scanState ? goalProjection.fedTargetWeight : goalProjection.cutTargetWeight) - goalProjection.currentWeight), 1),
      sub: `${scanState ? 'Fed-state' : 'Cut-state'} scale distance from current ${weightLabel(goalProjection.currentWeight)}`
    },
    {
      value: weightLabel(Math.abs(goalProjection.fedTargetWeight - goalProjection.cutTargetWeight), 1),
      sub: `Modeled fed-to-cut scale offset; non-fat mass, not extra fat loss`
    }
  ] : [{
      value: 'Need a cut',
  ...
  const compareRows = ['current', 'maintain', 'mild_cut', 'aggressive_cut', 'better_sleep', 'no_drinks'].map(key => {
    if (scenarioPlannerMode === 'goal') {
      ...
      return `
        <div class="compare-card${scenarioPreset === key ? ' active-scenario-compare' : ''}">
          ...
          <div class="tiny">${goalSnap?.achievable ? `${weightLabel(scanState ? goalSnap.fedTargetWeight : goalSnap.cutTargetWeight)} ${stateLabel}${scanState ? ` · ${weightLabel(goalSnap.cutTargetWeight)} cut eq.` : ` · ${weightLabel(goalSnap.fedTargetWeight)} fed`} · ${weightLabel(Math.abs(goalSnap.weeklyPace), 2)}/wk` : 'Current setup does not create a reliable downward pace to target.'}</div>
        </div>
      `;
    }
    ...
    return `
      <div class="compare-card${scenarioPreset === key ? ' active-scenario-compare' : ''}">
        ...
        <div class="delta">${weightLabel(snapshot.cutState.weight)} ${stateLabel}</div>
        <div class="tiny">${snapshot.scaleChange >= 0 ? '−' : '+'}${weightLabel(Math.abs(snapshot.scaleChange), 1)} scale · ${snapshot.fatChange >= 0 ? '−' : '+'}${weightLabel(Math.abs(snapshot.fatChange), 1)} fat${scanState ? '' : ` · ${weightLabel(snapshot.fedState.weight)} fed`}</div>
      </div>
    `;
  }).join('');
  ...
  if (scanState) document.getElementById('scenarioAssumptions').textContent = `Sep 23 fed-state DXA anchor: ${weightLabel(DXA_SCAN_LATEST.totalMass)}, 18.6% reported BF, ${weightLabel(DXA_SCAN_LATEST.fatMass)} fat, ${weightLabel(DXA_SCAN_LATEST.leanMass)} lean and ${weightLabel(DXA_SCAN_LATEST.boneMass)} bone. Creatine is already included. The cut-state equivalent removes ${weightLabel(DXA_SCAN_LATEST.fedStateDelta, 1)} of modeled non-fat scale mass, based on the scan versus the preceding 7-day average. Future loss is modeled as 95% fat / 5% fat-free mass. Working maintenance ~${energyLabel(scenarioTdee.maintenance)} from ${scenarioSourceText}.`;
  ...
  if (forecastCopy) {
    forecastCopy.textContent = scenarioPlannerMode === 'goal'
      ...
      : newerBodyCompScan(rangeDays) ? `Starts from the ${formatShortDate(DXA_SCAN_LATEST.date)} DXA measurement, newer than the last logged weigh-in, and compares your active scenario with current pace and core preset paths.`
